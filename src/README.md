...
- `/lager rein` und `/lager raus` haben die Felder `item1` bis `item20` — jedes im freien Format `Name Menge` (z. B. `Pistole 5`), `item1` ist Pflicht, `item2`–`item20` optional. (Discord erlaubt maximal 25 Optionen pro Command, deshalb ein kombiniertes Feld pro Item statt getrennter Name-/Mengen-Felder.) `/lager rein` legt Items an (neue Items werden automatisch angelegt), `/lager raus` nimmt sie raus (schlägt pro Feld einzeln fehl, wenn nicht genug Bestand da ist oder das Item nicht existiert — die restlichen Felder werden trotzdem verarbeitet). **Jeder Nutzer** darf beides, keine Rollen-Beschränkung — funktioniert aber ausschließlich in `lagerCommandChannelId`. Die Antwort listet für jedes Feld Erfolg oder Fehler einzeln auf.
